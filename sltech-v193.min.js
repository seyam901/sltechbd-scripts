// ==UserScript==
// @name         SL TECH BD V1.9.3 - Profit Format Fix + Slider Input Fix + Capital Input Removed
// @version      1.9.3
// @description  Demo balance fixed 10k, Live balance shows Demo real amount from specific element, tickmark move kore Live e | SL TECH BD
// @author       SL
...
(function () {
  ...
  function spoofUI() {
    ...
    const liveLi = listItems.find(li => li.innerText.includes("Live Account"));
    ...
    const fixedDemoAmountStr = formatAmount(10000);
  ...
  function updatePositionExpandOnProfitChange() {
    ...
    if (diff !== lastProfitDiff) {
      currentExpandPercent = Math.floor(Math.random() * 91) + 10; // 10% to 100%
      lastProfitDiff = diff;
